                ...
                PseudoInfiniteViewport.Pannable
{
     ...
     * 
     * @param g
     *            the Graphics object to use to render this component
     */
    public void paintComponent( Graphics g )
    {
        ...
        Color[] colors = { new Color( 163, 163, 209 ),
            new Color( 255, 170, 213 ), new Color( 255, 0, 255 ),
            new Color( 128, 0, 255 ) };
        Color[][] colorsOnGrid = {
            { colors[3], Color.white, Color.white, colors[1], Color.white,
                Color.white, Color.white, colors[3], Color.white, Color.white,
                Color.white, colors[1], Color.white, Color.white, colors[3] },
            { Color.white, colors[0], Color.white, Color.white, Color.white,
                colors[2], Color.white, Color.white, Color.white, colors[2],
                Color.white, Color.white, Color.white, colors[0], Color.white },
            { Color.white, Color.white, colors[0], Color.white, Color.white,
                Color.white, colors[1], Color.white, colors[1], Color.white,
                Color.white, Color.white, colors[0], Color.white, Color.white },
            { colors[1], Color.white, Color.white, colors[0], Color.white,
                Color.white, Color.white, colors[1], Color.white, Color.white,
                Color.white, colors[0], Color.white, Color.white, colors[1] },
            { Color.white, Color.white, Color.white, Color.white, colors[0],
                Color.white, Color.white, Color.white, Color.white,
                Color.white, colors[0], Color.white, Color.white, Color.white,
                Color.white },
            { Color.white, colors[2], Color.white, Color.white, Color.white,
                colors[2], Color.white, Color.white, Color.white, colors[2],
                Color.white, Color.white, Color.white, colors[2], Color.white },
            { Color.white, Color.white, colors[1], Color.white, Color.white,
                Color.white, colors[1], Color.white, colors[1], Color.white,
                Color.white, Color.white, colors[1], Color.white, Color.white },
            { colors[3], Color.white, Color.white, colors[1], Color.white,
        ...
        for ( int row = 0; row < 15; row++ )
        {
            ...
            {
                g2.setColor( colorsOnGrid[row][col] );
                g2.fillRect( insets.left + cellSize * col + col + 1, insets.top
                    + cellSize * row + row + 1, cellSize, cellSize );
     ...
     * 
     * @param g2
     *            the Graphics2D object to use to render
     * @param xleft
     *            the leftmost pixel of the rectangle
     * @param ytop
     *            the topmost pixel of the rectangle
     * @param obj
     *            the Locatable object to draw
     */
     ...
     * that intersect the current clipping bounds.
     * 
     * @param g2
    ...
    private void drawGridlines( Graphics2D g2 )
    {
        ...
        for ( int y = miny; y <= maxy; y += cellSize + 1 )
            ...
            {
                Location loc = locationForPoint( new Point( x + cellSize / 2, y
                    + cellSize / 2 ) );
     ...
     * 
     * @param flag
     *            true/false to enable/disable tool tips
     */
    ...
    /**
     * Sets the grid being displayed. Reset the cellSize to be the largest that
     * fits the entire grid in the current visible area (use default if grid is
     * too large).
     * 
     * @param gr
     *            the grid to display
    ...
    {
        return new Dimension( numCols * ( MIN_CELL_SIZE + 1 ) + 1
            + extraWidth(), numRows * ( MIN_CELL_SIZE + 1 ) + 1 + extraHeight() );
    }
     ...
     * This method is used by the GUI when creating Fish by clicking on cells in
     * the display.
     * 
     * @param p
     *            the Point in question (in display's coordinate system)
     * @return the Location beneath the event (which may not be a valid location
     *         in the grid)
     */
     ...
     * valid grid cell. we provide some information about the cell and its
     * contents. This method is automatically called on mouse-moved events since
     * we register for tool tips.
     * 
     * @param evt
    ...
    private String getToolTipText( Location loc )
    {
        ...
        if ( f != null )
            return MessageFormat.format( resources.getString( "cell.tooltip.nonempty" ),
                new Object[] { loc, f } );
        else
            return MessageFormat.format( resources.getString( "cell.tooltip.empty" ),
                new Object[] { loc, f } );
    }
     ...
     * 
     * @param dr
     *            the number of rows by which to move the location
     * @param dc
     *            the number of columns by which to move the location
     */
     ...
     *            the tool tip text
     * @param pt
     *            the pixel position over which to show the tip
     */
     ...
     * the number of rows and columns in the grid. We use the largest cellSize
     * that will fit in the viewable region, bounded to be at least the
     * parameter minSize.
     */
    private void recalculateCellSize( int minSize )
    {
        ...
        else
        {
            ...
            int desiredCellSize = Math.min( ( viewableSize.height - extraHeight() )
                / numRows,
    ...
    {
        return new Dimension( DEFAULT_CELL_COUNT * ( DEFAULT_CELL_SIZE + 1 )
            + 1 + extraWidth(), DEFAULT_CELL_COUNT * ( DEFAULT_CELL_SIZE + 1 )
            + 1 + extraHeight() );
    }
